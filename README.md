...
The delete modal has `role="dialog"`, `aria-modal="true"`, and `aria-labelledby` pointing to the modal title's `id`. The form panel has `role="dialog"` and `aria-label`. The filter button has `aria-haspopup="true"` and `aria-expanded` reflecting open state. The backdrop div has `aria-hidden="true"` so screen readers skips it.
